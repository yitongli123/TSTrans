...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    def save_bb(file, data):
        tracked_bb = np.array(data).astype(int)  # int -> float ?
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    _save_tracker_output(seq, tracker, output)  # 0728: 在可视化的同时保存test结果

    return num_frames / exec_time  # 1009
    ...
    if mode == 'sequential':
        ...
        for seq in dataset:
            for tracker_info in trackers:
                fps = run_sequence(seq, tracker_info, debug=debug)
                avg_fps += fps
        print('Average FPS:', avg_fps / len(dataset))
    elif mode == 'parallel':
