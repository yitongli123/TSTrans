...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    fps_all = torch.zeros((len(dataset), len(trackers)), dtype=torch.float64)
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            for results_dir in trk.results_dir:
            ...
            # Calculate fps
            time_results_path = '.'.join(results_path.split('.')[:-1])+'_time.txt'
            if not os.path.isfile(results_path):               
                raise Exception('Result not found. {}'.format(time_results_path))           
            seq_frame_num = 0
            seq_total_time = 0.0           
            with open(time_results_path, 'r') as f_time:
                time_list = f_time.readlines()
            for t in time_list:
                t = float(t.strip())
                seq_total_time += t
                seq_frame_num += 1
            fps_all[seq_id, trk_id] = seq_frame_num / seq_total_time
