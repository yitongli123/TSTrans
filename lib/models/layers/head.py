...
class CenterPredictor(nn.Module, ):
    ...
    def forward(self, x, gt_score_map=None):
        ...
        score_map_ctr, offset_map = self.get_score_map(x)

        # assert gt_score_map is None
        if gt_score_map is None:
            #bbox = self.cal_bbox(score_map_ctr, size_map, offset_map)
            center = self.cal_bbox(score_map_ctr, offset_map)
        else:
            #bbox = self.cal_bbox(gt_score_map.unsqueeze(1), size_map, offset_map)
            center = self.cal_bbox(gt_score_map.unsqueeze(1), offset_map)

        #return score_map_ctr, bbox, size_map, offset_map
        return score_map_ctr, center, offset_map

    def cal_bbox(self, score_map_ctr, offset_map, return_score=False):  # remove 'size_map'
        max_score, idx = torch.max(score_map_ctr.flatten(1), dim=1, keepdim=True)
        #idx_y = idx // self.feat_sz  # 0719
        idx_y = torch.div(idx, self.feat_sz, rounding_mode='floor')  # 0719
        ...
        # cx, cy
        center = torch.cat([(idx_x.to(torch.float) + offset[:, :1]) / self.feat_sz,
                          (idx_y.to(torch.float) + offset[:, 1:]) / self.feat_sz], dim=1)

        if return_score:
            #return bbox, max_score
            return center, max_score
        #return bbox
        return center
    ...
    def get_score_map(self, x):
        ...
        return _sigmoid(score_map_ctr), score_map_offset
