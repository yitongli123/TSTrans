    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            data['template_images'], data['template_anno'], data['template_masks'] = self.transform['joint'](
                image=data['template_images'], bbox=data['template_anno'], mask=data['template_masks'], joint=True)
            data['search_images'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_images'], bbox=data['search_anno'], mask=data['search_masks'], joint=True, new_roll=False)

        for s in ['template', 'search']:
            assert self.mode == 'sequence'
            if s == 'template':
                assert len(data[s + '_images']) == 2
            else:
                assert len(data[s + '_images']) == 1
            ...
            # lyt, add "resize_factor"
            crops, boxes, att_mask, mask_crops, resize_factor = prutils.jittered_center_crop(data[s + '_images'], jittered_anno,
                                                                            data[s + '_anno'], self.search_area_factor[s],
                                                                            self.output_sz[s], masks=data[s + '_masks'])
            data[s + '_resize_factor'] = resize_factor
